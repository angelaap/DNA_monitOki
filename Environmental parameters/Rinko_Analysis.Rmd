...
temp_all_p <- ggplot(mean_sd_rinko, aes(x=date, y=water_temp_Mean, color=Position)) +  facet_wrap(~ location, scales="free" )+ geom_point(size=3) + theme_bw() + theme(text = element_text(size=12)) +ylab("Water temperature (°C)") +xlab("") + theme(text = element_text(size=18)) + theme(legend.position="none")
