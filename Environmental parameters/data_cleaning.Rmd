...
## Add the columns with the ID information
...
```{r last tables Sept}
rinko_sept <- "~/Documents/GitHub/DNA_monitOki/Environmental parameters/Sept 21/26. September 21/"
rinko_csv_Sept <- 
  list.files(path = rinko_sept, recursive= TRUE, pattern = "*.csv", full.names=TRUE)
print (rinko_csv_Sept)

#test file
test2<- read.csv("~/Documents/GitHub/DNA_monitOki/Environmental parameters/Sept 21/26. September 21/R1C 202109020959_ASTD152-ALC-R02_0264_095919.csv", skip= 44)
colnames(test2) <- vars[c(2, 5, 6:19)]

#run script in the rest of the files
for (filecsv in rinko_csv_Sept) {
  df <- read.csv(filecsv, skip = 44)
  out_removed <- sapply(df [ ,3:16], remove_outliers_column) %>%
   bind_cols()
  out_removed_id <- bind_cols(df[1:2], out_removed)
  colnames(out_removed_id) <- vars[c(2, 5, 6:19)]

  write.csv(out_removed_id, gsub('.csv', '_processed.csv', filecsv))
}

#path to the separate folder that has the outlier-replaced-with-NA excel files
rinko_outliers_removed  <- "~/Documents/GitHub/DNA_monitOki/Environmental parameters/Sept 21/26. September 21/"


# list of csv files to loop through and bind together
rinko_sept_no_outliers <- 
  list.files(path = rinko_outliers_removed , recursive = TRUE, pattern = "*_processed.csv")
print(rinko_sept_no_outliers)

readBind <- function (x){
  read.csv(x)
}

setwd("~/Documents/GitHub/DNA_monitOki/Environmental parameters/Sept 21/26. September 21")
getwd()
all_outliers_removed <- sapply(rinko_sept_no_outliers, readBind, simplify=FALSE) %>%
  bind_rows(.id = "id") %>% 
...
# for some reason needs to be a dataframe to write an excel file with row.names = FALSE
write_all_out_removed <- as.data.frame(all_outliers_removed) 
write.xlsx(write_all_out_removed, file = "~/Documents/GitHub/DNA_monitOki/Environmental parameters/rinko_all_outliers_removed_Sept.xls", 
           colNames = TRUE, rowNames = FALSE)
