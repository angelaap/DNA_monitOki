...
#now calculate previous 24 hours
rain.wide$sumsGoya <- rollsumr(rain.wide$Goya, k = 144, fill = NA)
rain.wide$sumsKuniga <- rollsumr(rain.wide$Kuniga, k = 144, fill = NA)
rain.wide$sumsYomitan<- rollsumr(rain.wide$Yomitan, k = 144, fill = NA)
...
rain_wind<- rain_wind[complete.cases(rain_wind), ]
Urain_wind<- rain_wind %>% 
 filter(grepl("U",location.x))
numeric_accurateU<- Urain_wind[ , -c(1, 2, 10, 11, 12, 13, 20, 21, 22, 23, 25)]
res <- cor(numeric_accurateU) #get the cor matrix
corrplot(res, type = "upper", order = "hclust", tl.col = "black", tl.srt = 45) # correlation plot
```
...
rain_wind<- rain_wind[complete.cases(rain_wind), ]
Rrain_wind<- rain_wind %>% 
 filter(grepl("R",location.x))
numeric_accurateR<- Rrain_wind[ , -c(1, 2, 10, 11, 12, 13, 20, 21, 22, 23, 25)]
res <- cor(numeric_accurateR) #get the cor matrix
corrplot(res, type = "upper", order = "hclust", tl.col = "black", tl.srt = 45) # correlation plot
```
```{r}
U1rain_wind<- rain_wind%>% 
  filter(grepl("U1", sample))
U1rain_wind<- U1rain_wind[complete.cases(U1rain_wind), ]
numeric_accurateU1<- U1rain_wind[ , -c(1, 2, 10, 11, 12, 13, 20, 21, 22, 23, 25)]
numeric_accurateU1$tide<- as.numeric(numeric_accurateU1$tide)
res <- cor(numeric_accurateU1) #get the cor matrix
corrplot(res, type = "upper", order = "hclust", tl.col = "black", tl.srt = 45) # correlation plot
```
```{r}
U2rain_wind<- rain_wind%>% 
  filter(grepl("U2", sample))
U2rain_wind<- U2rain_wind[complete.cases(U2rain_wind), ]
numeric_accurateU2<- U2rain_wind[ , -c(1, 2, 10, 11, 12, 13, 20, 21, 22, 23, 25)]
numeric_accurateU2$tide<- as.numeric(numeric_accurateU2$tide)
res <- cor(numeric_accurateU2) #get the cor matrix
corrplot(res, type = "upper", order = "hclust", tl.col = "black", tl.srt = 45) # correlation plot
```
```{r}
R1rain_wind<- rain_wind%>% 
  filter(grepl("R1", sample))
R1rain_wind<- R1rain_wind[complete.cases(R1rain_wind), ]
numeric_accurateR1<- R1rain_wind[ , -c(1, 2, 10, 11, 12, 13, 20, 21, 22, 23, 25)]
numeric_accurateR1$tide<- as.numeric(numeric_accurateR1$tide)
res <- cor(numeric_accurateR1) #get the cor matrix
corrplot(res, type = "upper", order = "hclust", tl.col = "black", tl.srt = 45) # correlation plot
```

```{r}
R2rain_wind<- rain_wind%>% 
  filter(grepl("R2", sample))
R2rain_wind<- R2rain_wind[complete.cases(R2rain_wind), ]
numeric_accurateR2<- R2rain_wind[ , -c(1, 2, 10, 11, 12, 13, 20, 21, 22, 23, 25)]
numeric_accurateR2$tide<- as.numeric(numeric_accurateR2$tide)
res <- cor(numeric_accurateR2) #get the cor matrix
corrplot(res, type = "upper", order = "hclust", tl.col = "black", tl.srt = 45) # correlation plot
```
Correlation of R2 makes much more sense. It correlates positively with turbidity and somewhow rain and negatively with temperature and conductivity. We also get a positive correlation with tides (?). See if this makes sense at all.
