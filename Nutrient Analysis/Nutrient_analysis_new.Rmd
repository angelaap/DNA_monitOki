...
```{r violin general}
microMtotal<- read.csv("~/Documents/GitHub/DNA_monitOki/Nutrient Analysis/microMtotal.csv")
microMtotal<- microMtotal %>% 
  mutate(TN = rowSums(across(c(NO3, NO2, NH4))))
...
nutrient.labs <- c("NH4+", "NO2-", "NO3-", "PO43-", "SiO2", "TN")
names(nutrient.labs) <- c("NH4", "NO2", "NO3", "PO4", "SiO2", "TN")
...
ggplot(ribbonarea, aes(x=dates, y=Mean, colour=area, fill=area, group = 1 )) +
           scale_colour_manual(values = c("#7F8E39", "#42858C", "#D33B44", "#1d3554"),
                       name="Area",
                       breaks=c("R1", "R2", "U1", "U2"),
                       labels=c("Tancha", "Ogimi", "Ginowan", "Nago")) +
